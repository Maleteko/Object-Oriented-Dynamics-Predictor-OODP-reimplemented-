...
test_data_size = 16
single_training_batchsize = 16
training_batchsize = single_training_batchsize * mirrored_strategy.num_replicas_in_sync
...
object_detector_num = (1,3) #(dynamic, static)
horizon=32
...
def train_on_batch(images, next_images, actions, batch_size, is_train=True):
    ...
    #object detector
    objs = None
    for i, obj_model in enumerate(object_model):
        if i == 0:
  ...
  #[batch_size, 3, horizon, horizon]
    ...
    Recon_I = (1 - tf.expand_dims(objs[:, 0], -1)) * tf.cast(bg, "float32") + tf.expand_dims(objs[:, 0], -1) * tf.cast(images, "float32")
    Recon_nextI = (1 - tf.expand_dims(next_objs[:, 0], -1)) * tf.cast(next_bg, "float32") + tf.expand_dims(next_objs[:, 0], -1) * tf.cast(next_images, "float32")

    Pred_Ia, Pred_M_dynamic = STNmodule(Pred_delt, images, objs[:,0], 80, batch_size)

    dynamicproposal = tf.reduce_mean(tf.square(tf.cast(images, "float32") - bg), axis=3) >= 0.2
    ...
    prediction_loss = 100 * LOSS.prediction_loss(Pred_nextI, tf.cast(next_images, "float32"))
    recon_loss = 100 * LOSS.recon_loss(Recon_I, tf.cast(images, "float32"), Recon_nextI, tf.cast(next_images, "float32"))
    ...
    tf.summary.scalar("Dym Predict/x", Pred_delt[0,0], step=step)
    tf.summary.scalar("Dym Predict/y", Pred_delt[0,1], step=step)

    return ([bg_loss, pro_loss, entropie_loss, highway_loss, prediction_loss, recon_loss, consist_loss],
            {"bg": bg, "objs": objs, "next_objs": next_objs, "Pred_nextI": Pred_nextI})

def dist_train(images, next_images, actions):
    with tf.GradientTape(persistent=True) as tape:
        loss, _= train_on_batch(images, next_images, actions, training_batchsize)
        loss_sum = tf.nn.compute_average_loss(loss, global_batch_size=training_batchsize)
...
@tf.function
def distributed_train_step(images, next_images, actions):
  per_replica_losses= mirrored_strategy.run(dist_train, args=(images, next_images, actions))
  return [mirrored_strategy.reduce(tf.distribute.ReduceOp.SUM, per_replica_losses[0],axis=None),
            ...
            mirrored_strategy.reduce(tf.distribute.ReduceOp.SUM, per_replica_losses[5],axis=None),
            mirrored_strategy.reduce(tf.distribute.ReduceOp.SUM, per_replica_losses[6],axis=None),
            mirrored_strategy.reduce(tf.distribute.ReduceOp.SUM, per_replica_losses[7],axis=None)]
...
with mirrored_strategy.scope():
    try:
        bg_model = tf.keras.models.load_model("data/model/" + experiment_name +"/checkpoints/" + str(start_epoch-1) + "/bg_model.h5", compile=False)
    ...
    try:
        for i in range(sum(object_detector_num)):
           object_model.append(tf.keras.models.load_model("data/model/" + experiment_name +"/checkpoints/" + str(start_epoch-1) + "/object_model"+str(i)+".h5", compile=False))
    ...
    try:
        for i in range(object_detector_num[1]):
            dynamics_model.append(tf.keras.models.load_model("data/model/" + experiment_name +"/checkpoints/" + str(start_epoch-1) + "/dynamics_model"+str(i)+".h5", compile=False))
...
writer = tf.summary.create_file_writer("data/tensorboard/" + experiment_name)
with writer.as_default():
    for epoch in range(epoch_num):
        print("epoch " + str(epoch+1+start_epoch)+"/"+str(epoch_num+start_epoch))
        with tqdm.tqdm(total=batch_num) as bar:
            for image_path, next_image_path, actions in dataset:
                ...
                loss = distributed_train_step(images, next_images, actions)
                loss_sum, bg_loss, pro_loss, entropie_loss, highway_loss, prediction_loss, recon_loss, consist_loss = loss
        ...
        pathlib.Path("data/model/" + experiment_name +"/checkpoints/"+str(epoch+start_epoch)).mkdir(parents=True, exist_ok=True)
        bg_model.save("data/model/" + experiment_name +"/checkpoints/"+str(epoch+start_epoch)+"/bg_model.h5")
        for i, model in enumerate(object_model):
            model.save("data/model/" + experiment_name +"/checkpoints/"+str(epoch+start_epoch)+"/object_model"+str(i)+".h5")
        for i, model in enumerate(dynamics_model):
            model.save("data/model/" + experiment_name +"/checkpoints/"+str(epoch+start_epoch)+"/dynamics_model"+str(i)+".h5")

        validation()
pathlib.Path("data/model/" + experiment_name +"/").mkdir(parents=True, exist_ok=True)
bg_model.save("data/model/" + experiment_name +"/bg_model.h5")
for i, model in enumerate(object_model):
    model.save("data/model/" + experiment_name +"/object_model"+str(i)+".h5")
for i, model in enumerate(dynamics_model):
    model.save("data/model/" + experiment_name +"/dynamics_model"+str(i)+".h5")
